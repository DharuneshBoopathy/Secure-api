        ...
        <div className="overflow-hidden glass-card">
          <table className="min-w-full divide-y divide-ink-100 text-sm">
            ...
            <tbody className="divide-y divide-ink-100">
              {rows.map((r) => (
                <tr key={`${r.method}-${r.path_template}`} className="hover:bg-ink-50/60">
                  <td className="px-4 py-3">
                    <Badge variant="neutral">{r.method}</Badge>
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-ink-800 dark:text-ink-100">{r.path_template}</td>
                  <td className="px-4 py-3 text-ink-600 dark:text-ink-400">
                    {r.last_traffic ? new Date(r.last_traffic).toLocaleString() : (
                      <span className="text-warning-700">Never</span>
                    )}
                  </td>
